Un programme pour le projet MowItNow. Les spécifications sont expliquées ci-après.

Pour exécuter le programme, utiliser la classe MowItMain.
Pour modifier les données des tondeuses, utiliser le fichier resources/mowit.txt
...
du coin inférieur gauche sont supposées être (0,0) • La suite du fichier permet de piloter toutes les tondeuses qui ont été déployées. Chaque
tondeuse a deux lignes la concernant :
• la première ligne donne la position initiale de la tondeuse, ainsi que son orientation. La
position et l'orientation sont fournies sous la forme de 2 chiffres et une lettre, séparés
par un espace • la seconde ligne est une série d'instructions ordonnant à la tondeuse d'explorer la
